...
const AssignTechModal = {
    ...
    loadTechnicians() {
        fetch(BASE_URL + 'api/tecnicos/buscar') // Using existing or new API
            .then(res => res.json())
            .then(data => {
                // If API structure differs, adapt here.
                // Expecting: { id, nombre, apellido, active_tickets, status, specialty }
                // fallback mock if API not ready during dev
                if (data.error || !Array.isArray(data)) {
                    // Mocking for dev based on user request if API fails
                    this.technicians = [
                        { id: 101, nombre: "Carlos", apellido: "Ruiz", status: 'available', active_tickets: 1, specialty: 'Redes' },
                        { id: 102, nombre: "Ana", apellido: "Gomez", status: 'busy', active_tickets: 5, specialty: 'Hardware' },
                        { id: 103, nombre: "Miguel", apellido: "Diaz", status: 'available', active_tickets: 0, specialty: 'Software' }
                    ];
                } else {
            ...
            .catch(err => {
                console.error(err);
                this.renderPlaceholder('Error al cargar técnicos.');
            });
    },
