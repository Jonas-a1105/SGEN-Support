...
// Logic Override: If DB_PORT is explicitly set to 3306 in .env/environment, 
// we assume standard web mode even if other heuristics suggest DesktopApp
if (env_get('DB_PORT') === '3306') {
    $isDesktopApp = false;
}

// For desktop app, FORCE these values (using different constant names to avoid conflicts)
if ($isDesktopApp) {
    // APP_MODE is passed from Electron main process
    $appMode = env_get('APP_MODE', 'server');
    $dbHost = env_get('DB_HOST', '127.0.0.1');

    // For desktop app, we MUST use these values regardless of what .env says
    if (!defined('DB_HOST')) define('DB_HOST', $dbHost);
    if (!defined('DB_PORT')) define('DB_PORT', env_get('DB_PORT', '3307'));
    ...
    file_put_contents(__DIR__ . '/../public/db_debug.log', "  After define: DB_HOST=" . DB_HOST . ", DB_PORT=" . DB_PORT . "\n", FILE_APPEND);
} else {
