        ...
        <div class="tf-header-edit">
            ...
            <div class="tf-header-actions">
                <a href="<?= BASE_URL ?>soportes/eliminar/<?= $soporte->id ?>" 
                   ...
                   onclick="event.preventDefault(); event.stopPropagation(); SimpleDeleteModal.open(this.href, {
                       type: 'Ticket #<?= $soporte->id ?>',
                       name: '<?= addslashes(htmlspecialchars(str_replace(["\r", "\n"], ' ', substr($soporte->descripcion ?? '', 0, 60)))) ?>...',
                       warning: 'Se eliminarán todos los archivos y comentarios asociados.'
                   }); return false;">
...
<!-- Modular Ticket Form Script -->
<script src="<?= BASE_URL ?>js/ticket-form.js?v=<?= time() ?>"></script>